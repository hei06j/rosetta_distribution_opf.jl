...
# control_forming = "droop"
        ...
        if haskey(bus, "type")
            if bus["type"] == "GFM"
                ...
                if control_forming == "setpoint"
                    JuMP.@constraint(model, [t in terminals], vm[t] == bus["vm"][t])
                ...
                elseif control_forming == "droop"
                    ...
                    gen_id = 1
                    gen = ref[:gen][gen_id]
                    bus_id = [branch["t_bus"] for (id, branch) in ref[:branch] if branch["f_bus"]==i][1]
                    vm_id = JuMP.@variable(model, [t in terminals], base_name="vm_$bus_id", lower_bound=0)
                    JuMP.@constraint(model, [t in terminals], vm_id[t]^2 == vr[t,bus_id]^2 + vi[t,bus_id]^2)
                    JuMP.@constraint(model, [p in phases], vm_id[p] - bus["vm"][p] ==  gen["Dp"][p]*(gen["pg"][p] - pg[p,gen_id]) + gen["Dq"][p]*(gen["qg"][p] - qg[p,gen_id]))
    ...
    for (id, generator) in ref[:gen]
        ...
        if configuration==PMD.WYE || length(pmin)==1 || nphases==1
            ...
            if !isempty(nonInf_pmin_pmax)
                ...
                JuMP.@constraint(model, sum(pmin[nonInf_pmin_pmax]) .<= sum(pg[nonInf_pmin_pmax,id]) .<= sum(pmax[nonInf_pmin_pmax]) )
            end
    ...
    # inv_bus_id = 3
    # terminals = ref[:bus][inv_bus_id]["terminals"]
    # vm = JuMP.@variable(model, [t in terminals], base_name="vm_$inv_bus_id", lower_bound=0)
    # JuMP.@constraint(model, [t in terminals], vm[t]^2 == vr[t,inv_bus_id]^2 + vi[t,inv_bus_id]^2)

    # load_bus_id = 1
    # terminals = ref[:bus][load_bus_id]["terminals"]
    # vm_d = JuMP.@variable(model, [t in terminals], base_name="vm_$load_bus_id", lower_bound=0)
    # JuMP.@constraint(model, [t in terminals], vm_d[t]^2 == vr[t,load_bus_id]^2 + vi[t,load_bus_id]^2)

    # cmg = JuMP.@variable(model, [t in 1:4], base_name="cmg", lower_bound=0)
    # JuMP.@constraint(model, [t in 1:4], cmg[t]^2 == crg_bus[t,1]^2 + cig_bus[t,1]^2)

    # kp = 2E-4
    # crating = 7.5
    # JuMP.@constraint(model, cmg .+ kp .* (vm .- vm_d) .<= crating)
