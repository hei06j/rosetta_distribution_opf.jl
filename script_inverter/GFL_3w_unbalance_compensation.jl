...
# for objective in ["cost", "loss", "VUF", "VUF2", "PVUR", "LVUR", "IUF", "IUF2", "PIUR"]
# for objective in ["cost"]
    ...
    JuMP.@constraint(model, sum(Array(crg_bus[1:3,1]))==0)
    JuMP.@constraint(model, sum(Array(cig_bus[1:3,1]))==0)
    ...
    results_GFL_3w[objective]["v_gen"] = v1
    results_GFL_3w[objective]["v_gen_012"] = v1_012
    results_GFL_3w[objective]["c_gen"] = cg1
    results_GFL_3w[objective]["c_gen_012"] = cg1_012
    results_GFL_3w[objective]["c_load"] = cd1
    results_GFL_3w[objective]["c_load_012"] = cd1_012
    results_GFL_3w[objective]["c_branch"] = c1
    results_GFL_3w[objective]["c_branch_012"] = c1_012
    results_GFL_3w[objective]["sg_gen"] = Array(value.(pg)[:,1] .+ im * value.(qg)[:,1])
    results_GFL_3w[objective]["sg_source"] = Array(value.(pg)[:,2] .+ im * value.(qg)[:,2])
...
include("./GFL_4w_unbalance_compensation.jl")
...
# v_zero_seq = [results_GFL_3w[obj]["v_gen_012"][1] for obj in objectives]
# v_positive_seq = [results_GFL_3w[obj]["v_gen_012"][2] for obj in objectives]
# v_negative_seq = [results_GFL_3w[obj]["v_gen_012"][3] for obj in objectives]
# cg_zero_seq = [results_GFL_3w[obj]["c_gen_012"][1] for obj in objectives]
# cg_positive_seq = [results_GFL_3w[obj]["c_gen_012"][2] for obj in objectives]
# cg_negative_seq = [results_GFL_3w[obj]["c_gen_012"][3] for obj in objectives]

# scatter(objectives, v_zero_seq, label="V0", xticks=(0.5:7.5, objectives))
# scatter!(objectives, v_negative_seq, label="V2")

# scatter(objectives, cg_zero_seq, label="Ig0", xticks=(0.5:7.5, objectives))
# scatter!(objectives, cg_negative_seq, label="Ig2")
