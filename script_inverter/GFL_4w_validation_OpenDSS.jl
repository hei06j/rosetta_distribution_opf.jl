...
ODSS.dss("Redirect ./data/test_gen_4w_wye_InvControl.dss")
...
cm_Control = ODSS.PDElements.AllPowers()
ODSS.PDElements.AllCurrentsAllCurrents()

# plot([ones(3) 2*ones(3)], [vm2_noControl[1:3] vm2_Control[1:3]], seriestype=:scatter)
...
##
data_path = "./data/test_gen_4w_wye.dss"
...
vm_range = 0.9:0.05:1.1
vw_curve(v, pmax) = v <= 0.95 ? pmax : (v >= 1.05 ? 0 : -pmax/0.1*(v-1.05))
vw_curve_plot = plot(vm_range, vw_curve.(vm_range,11.6), label="Volt-Watt curve", guidefontsize=10)
xlabel!("Voltage Magnitude (V pu)")
ylabel!("Avtive Power (kW)")
savefig(vw_curve_plot, "./Figures/vw_curve_plot.pdf")

vm_range = 0.9:0.05:1.1
vv_curve_plot = plot(vm_range, vv_curve.(vm_range,6.45), label="Volt-var curve", guidefontsize=10)
xlabel!("Voltage Magnitude (V pu)")
ylabel!("Reavtive Power (kvar)")
savefig(vv_curve_plot, "./Figures/vv_curve_plot.pdf")