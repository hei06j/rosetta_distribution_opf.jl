...
data_math["bus"]["$new_bus_id"]["vm"] = [1.02, 1.02, 1.02, 0, 0]
data_math["bus"]["$new_bus_id"]["va"] = [0, -120, 120, 0, 0] .* pi/180
data_math["bus"]["$new_bus_id"]["vmin"] = copy(data_math["bus"]["$new_bus_id"]["vm"])
data_math["bus"]["$new_bus_id"]["vmax"] = copy(data_math["bus"]["$new_bus_id"]["vm"])

sourcebus_branches = [i for (i, branch) in data_math["branch"] if branch["f_bus"]==sourcebus]
source2branch = sourcebus_branches[1]
data_math["branch"]["$source2branch"]["f_bus"] = new_bus_id
...
data_math_no_sop = deepcopy(data_math)
data_math_no_sop, IM1_bus, IM2_bus, IM3_bus = make_case(data_math_no_sop; combined=true)
