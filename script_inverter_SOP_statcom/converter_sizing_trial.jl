...
# ipopt_solver = JuMP.optimizer_with_attributes(Ipopt.Optimizer, "sb"=>"yes", "max_iter"=>100000)

data_path = "./data/inverter_4w_wye_unbalanced_loads_2bus.dss"

function build_inverter_case(data_eng, sratings, pdcratings; setting=nothing)
    ...
    for gen_id in pv_gen_ids
        ...
        data_math["gen"][gen_id]["srating_min"] = sratings[1]
        data_math["gen"][gen_id]["srating_max"] = sratings[2]

        data_math["gen"][gen_id]["pdcrating_min"] = pdcratings[1]
        data_math["gen"][gen_id]["pdcrating_max"] = pdcratings[2]
    end
...
function build_data_math(data_path; sbase=1, setting=nothing, sratings=[20, 80], pdcratings=[20, 80])
    ...
    data_math = build_inverter_case(data_eng, sratings, pdcratings; setting=setting)
    ...
    for (i,bus) in data_math["bus"]
        bus["vmin"] = [0.9, 0.9, 0.9, 0]
        bus["vmax"] = [1.1, 1.1, 1.1, 1.1]
    end
    ...
    # Sbase = data_math["settings"]["sbase"]
    ...
    Ibase = (sbase * sbase_factor) / (vbase * vbase_factor)  #[kA]
...
mn_data = IM.replicate(data_math, 2, PMD._pmd_global_keys)
...
hours = 1:length(pd_load_a)
p_pd = plot(hours, pd_load_a, label="Pd A", lw=2)
plot!(p_pd, hours, pd_load_b, label="Pd B", lw=2)
plot!(p_pd, hours, pd_load_c, label="Pd C", lw=2)
# total load across the three phases
pd_average = (pd_load_a .+ pd_load_b .+ pd_load_c)/3
plot!(p_pd, hours, pd_average, label="Pd Average", lw=3, ls=:dash, color=:black)
xlabel!("Hour"); ylabel!("Pd (units)")
display(p_pd)

# Loop over each time step and assign corresponding pd and qd values
for (nw_key, nw_dict) in mn_data["nw"]
    time_step = parse(Int, nw_key)

    nw_dict["load"]["1"]["pd"] = pd_load_a[time_step]
    nw_dict["load"]["1"]["qd"] = qd_load_a[time_step]

    nw_dict["load"]["2"]["pd"] = pd_load_b[time_step]
    nw_dict["load"]["2"]["qd"] = qd_load_b[time_step]

    nw_dict["load"]["3"]["pd"] = pd_load_c[time_step]
    nw_dict["load"]["3"]["qd"] = qd_load_c[time_step]
end
