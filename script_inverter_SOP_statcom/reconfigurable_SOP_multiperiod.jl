...
### updates load and voltage source data from timeseries data
### data_ts is a dictionary of timeseries data
function update_timeseries_data!(data_math, data_ts, timestep; power_factor=0.9)
    for (i, load) in data_math["load"]
        load["pd"] = data_ts["load"][i]["pd"][timestep]
        load["qd"] = load["pd"] * tan(acos(power_factor))
    end
    ref_bus = [i for (i,bus) in data_math["bus"] if bus["bus_type"]==3][1]
    data_math["bus"]["$ref_bus"]["vm"] = data_ts["vsource"]["vm"][timestep]
end

##
### parse data and build engineering and mathematical data models
...
##
timesteps = 1:10
data_conv_mn = Dict("nw"=>Dict(string(i)=>Dict() for i in 1:length(timesteps)))
result_conv_mn = Dict("solution"=>Dict("nw"=>Dict(string(i)=>Dict() for i in 1:length(timesteps))))
data_mx_mn = Dict("nw"=>Dict(string(i)=>Dict() for i in 1:length(timesteps)))
result_mx_mn = Dict("solution"=>Dict("nw"=>Dict(string(i)=>Dict() for i in 1:length(timesteps))))
data_ideal_mn = Dict("nw"=>Dict(string(i)=>Dict() for i in 1:length(timesteps)))
result_ideal_mn = Dict("solution"=>Dict("nw"=>Dict(string(i)=>Dict() for i in 1:length(timesteps))))
...
for t = 1:10
    ...
    result_conv_mn["solution"]["nw"]["$h"] = result_conv["solution"]
    result_conv_mn["solution"]["nw"]["$h"]["objective"] = result_conv["objective"]
    # data_mn["nw"]["$h"] = deepcopy(data)
    ...
    result_mx_mn["solution"]["nw"]["$h"] = result_mx["solution"]
    result_mx_mn["solution"]["nw"]["$h"]["objective"] = result_mx["objective"]
    ...
    result_ideal_mn["solution"]["nw"]["$h"] = result_ideal["solution"]
    result_ideal_mn["solution"]["nw"]["$h"]["objective"] = result_ideal["objective"]
