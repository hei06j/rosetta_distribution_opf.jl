...
ref = IM.build_ref(data_math, _PMD.ref_add_core!, _PMD._pmd_global_keys, _PMD.pmd_it_name)[:it][:pmd][:nw][0]
...
for (id, generator) in ref[:gen]
    ...
    if configuration==_PMD.WYE || length(pmin)==1 || nphases==1
        ...
        # crg_bus[id] = RPMD._merge_bus_flows(model, [crg[id]..., -sum(crg[id])], connections)
        # cig_bus[id] = RPMD._merge_bus_flows(model, [cig[id]..., -sum(cig[id])], connections)
        crg_bus[id] = JuMP.Containers.DenseAxisArray([crg[id]..., -sum(crg[id])], connections)
        cig_bus[id] = JuMP.Containers.DenseAxisArray([cig[id]..., -sum(cig[id])], connections)
        ...
        crg_bus[id] = RPMD._merge_bus_flows(model, Md'*crg[id], connections)
        cig_bus[id] = RPMD._merge_bus_flows(model, Md'*cig[id], connections)
        RPMD._merge_bus_flows(model, Md'*crg[id], connections)
        RPMD._merge_bus_flows(model, Md'*cig[id], connections)
...
for (i, branch) in ref[:branch]
    ...
    # cr_bus[f_idx] = RPMD._merge_bus_flows(model, cr_fr, f_connections)
    # ci_bus[f_idx] = RPMD._merge_bus_flows(model, ci_fr, f_connections)
    ...
    # cr_bus[t_idx] = RPMD._merge_bus_flows(model, cr_to, t_connections)
    # ci_bus[t_idx] = RPMD._merge_bus_flows(model, ci_to, t_connections)
...
for (id, load) in ref[:load]
    ...
    if configuration==_PMD.WYE || int_dim==1
        ...
        # crd_bus[id] = RPMD._merge_bus_flows(model, [crd[id]..., crd_bus_n], connections)
        # cid_bus[id] = RPMD._merge_bus_flows(model, [cid[id]..., cid_bus_n], connections)
        crd_bus[id] = JuMP.Containers.DenseAxisArray([crd[id]..., crd_bus_n], connections)
        cid_bus[id] = JuMP.Containers.DenseAxisArray([cid[id]..., cid_bus_n], connections)
        ...
        crd_bus[id] = RPMD._merge_bus_flows(model, crd_bus_expr, connections)
        cid_bus[id] = RPMD._merge_bus_flows(model, cid_bus_expr, connections)
    end
...
for (i, transformer) in ref[:transformer]
    ...
    if configuration == _PMD.WYE  || length(crt_fr)==1
        ...
        crt_bus[f_idx] = RPMD._merge_bus_flows(model, [crt_fr..., -sum(crt_fr)], f_connections)
        cit_bus[f_idx] = RPMD._merge_bus_flows(model, [cit_fr..., -sum(cit_fr)], f_connections)
        crt_bus[t_idx] = RPMD._merge_bus_flows(model, [crt_to..., -sum(crt_to)], t_connections)
        cit_bus[t_idx] = RPMD._merge_bus_flows(model, [cit_to..., -sum(cit_to)], t_connections)
    ...
    elseif configuration == _PMD.DELTA
        ...
        JuMP.@constraint(model, scale.*crt_fr .+ crt_to .== 0)
        JuMP.@constraint(model, scale.*cit_fr .+ cit_to .== 0)
        crt_bus[f_idx] = RPMD._merge_bus_flows(model, Md'*crt_fr, f_connections)
        cit_bus[f_idx] = RPMD._merge_bus_flows(model, Md'*cit_fr, f_connections)
        crt_bus[t_idx] = RPMD._merge_bus_flows(model, [crt_to..., -sum(crt_to)], t_connections)
        cit_bus[t_idx] = RPMD._merge_bus_flows(model, [cit_to..., -sum(cit_to)], t_connections)
...
for (i, bus) in ref[:bus]
    ...
    for (idx, t) in ungrounded_terminals
        RPMD.@smart_constraint(model,  [cr, crd, crg, crt, vr[i]],
        ...
        RPMD.@smart_constraint(model, [ci, cid, cig, cit, vi[i]],
