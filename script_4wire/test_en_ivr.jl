...
include("../src/4wire_IVR/RPMD/variables.jl")
...
for (id, generator) in ref[:gen]
    ...
    if configuration==_PMD.WYE || length(pmin)==1 || nphases==1
        ...
        crg_bus[id] = _RPMD._merge_bus_flows(model, Md'*crg[id], connections)
        cig_bus[id] = _RPMD._merge_bus_flows(model, Md'*cig[id], connections)
...
for (id, load) in ref[:load]
    ...
    if configuration==_PMD.WYE || int_dim==1
        ...
        crd_bus[id] = _RPMD._merge_bus_flows(model, [crd[id]..., crd_bus_n], connections)
        cid_bus[id] = _RPMD._merge_bus_flows(model, [cid[id]..., cid_bus_n], connections)
...
for (i, transformer) in ref[:transformer]
    ...
    elseif configuration == _PMD.DELTA
        ...
        JuMP.@constraint(model, scale.*crt_fr .+ crt_to .== 0)
        JuMP.@constraint(model, scale.*cit_fr .+ cit_to .== 0)
        crt_bus[f_idx] = _RPMD._merge_bus_flows(model, Md'*crt_fr, f_connections)
        cit_bus[f_idx] = _RPMD._merge_bus_flows(model, Md'*cit_fr, f_connections)
        crt_bus[t_idx] = _RPMD._merge_bus_flows(model, [crt_to..., -sum(crt_to)], t_connections)
        cit_bus[t_idx] = _RPMD._merge_bus_flows(model, [cit_to..., -sum(cit_to)], t_connections)
...
##
_PMD.add_start_vrvi!(data_math)
pm = _PMD.instantiate_mc_model(data_math, _PMD.IVRENPowerModel, build_mc_opf);
