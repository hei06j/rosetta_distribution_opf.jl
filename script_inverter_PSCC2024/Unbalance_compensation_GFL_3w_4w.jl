...
function get_solutions!(model, results, objective)
    ...
    crg_values = JuMP.value.(crg_bus)
    cig_values = JuMP.value.(cig_bus)
    cg0 = crg_values[:,source_id] + im * cig_values[:,source_id]
    cgm0 = abs.(cg0)
    cg0_012 = T * cg0[1:3]
    cgm0_012 = abs.(cg0_012)
    # cga1 = angle.(cg1).*180/pi
    ...
    results[objective]["c_source"] = cg0
    results[objective]["c_source_012"] = cg0_012
...
for gen_id in pv_gen_ids
    @show gen_id
    RPMD.add_inverter_losses!(data_math, gen_id; three_wire=true, multiplexing=false)
end
...
results_GFL_3w = Dict()
get_solutions!(model, results_GFL_3w, objective)
...
Imax = maximum(abs.(results_GFL_3w[key]["c_load"]))
I0_3w = round(abs(results_GFL_3w[key]["c_branch_012"][1]), digits=2)
I2_3w = round(abs(results_GFL_3w[key]["c_branch_012"][3]), digits=2)
GFL_3w_vuf_c = RPMD.plot_phasors(results_GFL_3w[key]["c_branch"], Imax; I2=I2_3w, I0=I0_3w)
Plots.savefig(GFL_3w_vuf_c, "./Figures/GFL_3w_vuf_c.pdf")
GFL_3w_vuf_cg = RPMD.plot_phasors(results_GFL_3w[key]["c_gen"], Imax)
Plots.savefig(GFL_3w_vuf_cg, "./Figures/GFL_3w_vuf_cg.pdf")
GFL_3w_vuf_cd = RPMD.plot_phasors(results_GFL_3w[key]["c_load"], Imax)
...
GFL_4w_vuf_c = RPMD.plot_phasors(results_GFL_4w[key]["c_branch"], Imax; I2=I2_4w, I0=I0_4w)
Plots.savefig(GFL_4w_vuf_c, "./Figures/GFL_4w_vuf_c.pdf")
GFL_4w_vuf_cg = RPMD.plot_phasors(results_GFL_4w[key]["c_gen"], Imax)
Plots.savefig(GFL_4w_vuf_cg, "./Figures/GFL_4w_vuf_cg.pdf")
GFL_4w_vuf_cd = RPMD.plot_phasors(results_GFL_4w[key]["c_load"], Imax, labeled=true)
